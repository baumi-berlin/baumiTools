def Calculate_Landsat_Seasonal_Median(year, startMonth, endMonth, roi=None, path=None, row=None, mask_clouds=True, mask_water=False, mask_snow=False, mask_fill=True, harmonize_l8=True):
	'''
...
def Calculate_S2_Seasonal_Median(year, startMonth, endMonth, roi=None, resolution=10):
	...
	if roi != None:
		s2_collect = ee.ImageCollection('COPERNICUS/S2_SR').filterBounds(roi).filter(ee.Filter.date(start, end)).select(bands, band_names).filter(ee.Filter.lt('CLOUDY_PIXEL_PERCENTAGE', 50)).map(mask_clouds)
	else:
